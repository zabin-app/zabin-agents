...
| End-to-end per-client installation | [docs/INSTALL.md](docs/INSTALL.md) |
...
- Keep one contract per role or workflow. [`skills/conductor`](skills/conductor/README.md) is the single conductor; host-specific translation belongs in an adapter note beside the skill it serves ([`skills/conductor/references/claude.md`](skills/conductor/references/claude.md) for Claude Code), never in a forked copy, and retired contracts stay uninstalled under [`attic/`](attic/). [`adapters/claude`](adapters/claude) carries only the Claude-only extras a portable skill would never hold — the Workflow-tool payload programs and their capability/vocabulary mapping — never pipeline discipline itself.
