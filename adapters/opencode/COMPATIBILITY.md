...
installation state (`/home/ed/.config/opencode`, `/home/ed/.local/share/opencode`,
`/home/ed/.cache/opencode`) and, because the CWD-independent project
...
independently verified, before and after, to leave the real `/home/ed`
...
| 3b | External skills auto-loaded from `~/.claude/skills` and `~/.agents/skills` | **DOC-CITED** (corroborated incidentally) | Documented verbatim in the bundled `customize-opencode` skill body: `"External skills (auto-loaded) | ~/.claude/skills/<name>/SKILL.md, ~/.agents/skills/<name>/SKILL.md"`. Not independently probed as a standalone fixture (out of scope: this audit's project-level skill probe already exercises the mechanism), but incidentally corroborated by the containment-incident log line `"duplicate skill name" name=doc_validate existing=/home/ed/.claude/skills/... duplicate=/home/ed/.agents/skills/...` produced by a real invocation against the operator's real trees. |
