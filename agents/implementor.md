...
**Repair path for a partial composite.** A composite stops at its first failing step and its `steps_committed` flags report truthfully which steps landed; it never rolls back what already committed. Do not blindly retry a partial composite — re-running `start_task` against an already-claimed lease or an already-registered worktree is not the recovery path. Instead, read `steps_committed`, then finish the remaining steps with the singular tools under the same held lease: if `start_task` reports the claim and card fetch landed but `register_worktree` did not, call `register_worktree` directly with the same task, agent, path, branch, base branch, and base SHA, then `update_task_status` to `executing` if that step is also unmarked. If `finish_task` reports `record_commits` landed but `record_task_summary` did not, call `record_task_summary` directly, then `update_worktree_status(..., "idle")` and `update_task_status(..., "in_review")` for any step `steps_committed` still shows as missing. The held lease authorizes every singular repair call; a partial composite is never grounds to re-claim or release.
